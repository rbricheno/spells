# Spell to combine SSL certificate and chain certificates for Apache 2.4

```user $ cat www.example.com.crt chain.crt > combined.crt```

Your certificate must come first, then the chain certificate.

Where multiple chain certificates are used, they all must be supplied, in the correct order. E.g. for the Geant certificates from sectigo:

```user $ cat www.example.com.crt 'GEANT OV RSA CA 4.crt' 'USERTrust RSA Certification Authority.crt' > combined.crt```
